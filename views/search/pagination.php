        ...
        <ul id="search-pagination" class="pagination" >
            ...
            for ($i = $min; $i <= min($max, ceil($searchBuilder->getSelectedTotalRows() / $searchBuilder->getRowsPerPage())); $i++) {
                ...
                <li <?php echo $activeFlag; ?>>
                    <a id="<?php echo "page_".$i;?>" href=" <?php echo $searchView->getUrlByOffset($i); ?>" aria-label="up_page number"><?php echo $i ?></a>
                </li>
            <?php } ?>
            <li>
...
function partialPagination_down($searchBuilder, $searchView)
{
    $min = max(1, $searchBuilder->getOffset() - 5);
    $min = min($min, ceil($searchBuilder->getSelectedTotalRows() / $searchBuilder->getRowsPerPage()) - 10);
    $min = max($min, 1);
    ...
    ?>
    <div class="pull-left">
        <ul id="down_search-pagination" class="pagination" >
            <li>
                <a id="down_page_First" href="<?php echo $searchView->getUrlByOffset(1) ?>" aria-label="Previous">
                    <span aria-hidden="true">First</span>
                </a>
            </li>
            <li>
                <a id="down_page_Previous" href="<?php echo $searchView->getUrlByOffset($searchView->get_previsoue($searchBuilder->getOffset())); ?>"
                   aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            ...
            for ($i = $min; $i <= min($max, ceil($searchBuilder->getSelectedTotalRows() / $searchBuilder->getRowsPerPage())); $i++) {
                ...
                ?>
                <li <?php echo $activeFlag; ?>>
                    <a id="<?php echo "down_page_".$i;?>" href=" <?php echo $searchView->getUrlByOffset($i); ?>" aria-label="down_page number"><?php echo $i ?></a>
                </li>
            <?php } ?>
            <li>
                <a id="down_page_Next" href="<?php echo $searchView->getUrlByOffset($searchView->get_next($searchBuilder->getOffset(), $searchBuilder->getSelectedTotalRows(), $searchBuilder->getRowsPerPage())) ?>"
                   aria-label="Next">
                    <span aria-hidden="true">&raquo;</span>
                </a>
            </li>
            <li>
                <a id="down_page_Last" href="<?php echo $searchView->getUrlByOffset(ceil($searchBuilder->getSelectedTotalRows() / $searchBuilder->getRowsPerPage())) ?>"
                   aria-label="Next">
                    <span aria-hidden="true">Last</span>
                </a>
            </li>
